"use strict";
//jQuery time
var current_fs, next_fs, previous_fs; //setss
var left, opacity, scale; //sets properties which we will animate
...
$(".next").click(function(){
  ...
  $("#progressbar li").eq($("sets").index(next_fs)).addClass("active");
  
  //show the next sets
  next_fs.show(); 
  //hide the current sets with style
...
$(".previous").click(function(){
  ...
  $("#progressbar li").eq($("sets").index(current_fs)).removeClass("active");
  
  //show the previous sets
  previous_fs.show(); 
  //hide the current sets with style
  current_fs.animate({opacity: 0}, {
    step: function(now, mx) {
...
$("sets").delegate(".removeOrder", "click", function () {  
